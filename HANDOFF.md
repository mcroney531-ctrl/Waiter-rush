...
**This board is interim, and its replacement is already built.** `tools/render_room.mjs` assembles the room from the ten Meshy props and is live behind `?board=3d`, which switches the board image and every layout constant below together. The geometry in this section describes the *painted* board and stops being true the moment that flag becomes the default — see `art-source/ROOM-RENDER.md`. The spec asks for matching proportions specifically so the geometry above is mapped once rather than twice. Start at `art-source/ROOM-BRIEF.md`.
